...
async function dueCountText() {
  try {
    const { listDueCards } = await import('../engine/storage.js')
...
export function consumePendingReview(startDue: () => void): boolean {
  // the caller decides when the app is ready (post-boot)
  if (!localStorage.getItem(REVIEW_NOW_FLAG)) return false
  localStorage.removeItem(REVIEW_NOW_FLAG)
  setTimeout(startDue, 0)
  return true
}
...
export async function maybeScheduleReminders() {
  try {
    ...
    if (cap && cap.isNativePlatform && cap.isNativePlatform()) {
      ...
      await (LocalNotifications as any).cancelPending?.()
