...
Return ONLY valid JSON.`,{json:!0,maxOutputTokens:8e3,temperature:.2,timeoutMs:95e3})))}catch{}}catch(a){let d=String(a?.message||a);if(d.includes(`no_keys_configured`)||d.includes(`origin_not_allowed`))return{error:`relay_unavailable`};/timeout|upstream_timeout|504|abort/i.test(d)&&(c=!0)}}if(!l)return{error:c?`timeout`:`generation_failed`};if(n.length>1){let s$2=Array.isArray(l.gaps)?l.gaps:[];s$2.unshift(`This file is long \u2014 the reviewer was built from part ${m+1} of ${n.length}; later sections may be missing.`),l=s(r({},l),{gaps:s$2.slice(0,6)})}try{await $t(r$1.id,{reviewerAI:l})}catch{}return{reviewer:l,cached:!1}}function Ie(r,t){let e=t,i=[];i.push(`
    <div class="rvw-head">
