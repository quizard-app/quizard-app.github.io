import{l as e,o as t,t as n}from"./topics-CMOvnQe4.js";import{G as r,H as i,S as a,dt as o,gt as s,j as c,p as l,q as u}from"./index-CcyAXynw.js";function d(r,i=[]){let a=r.text,o=e(a),s=t(a),{membership:c}=n(a),l=o;if(i.length){let e=new Set(i.map(e=>e.toLowerCase()));l=o.filter(t=>{let n=c.get(t);return n&&e.has(n.toLowerCase())})}let u=l.join(` `),d=i.length&&l.length?t(u):s,p=l.map(e=>e.toLowerCase()),m=[],h=new Set;for(let e of d.slice(0,30)){let t=e.term,n=[];for(let e=0;e<l.length;e++)h.has(l[e])||new RegExp(t.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),`i`).test(p[e])&&n.push({text:l[e],index:e});if(!n.length)continue;let r=n.sort((e,t)=>e.text.length-t.text.length).slice(0,2);if(r.forEach(e=>h.add(e.text)),m.push({front:(e.phrase,f(e.term)),back:r.map(e=>e.text).join(` `),term:e.term}),m.length>=25)break}return m}function f(e){return e.charAt(0).toUpperCase()+e.slice(1)}async function p(e,t){let n=await i(t.state.currentDocId);if(!n){t.go(`library`);return}let f=d(n,[]);if(f.length<3){e.innerHTML=`
