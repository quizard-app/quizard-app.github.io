import{A as e,B as t,F as n,I as r,J as i,S as a,X as o,_t as s,lt as c,mt as l,p as u,rt as d,ut as f}from"./index-CcyAXynw.js";var p=[`#C4713B`,`#1A7F37`,`#0A66C2`,`#7C5CBF`,`#B54708`,`#475569`];async function m(e,t){let n=t.state.accountFlow?.mode||`picker`;return n===`create`?y(e,t):n===`lock`?x(e,t):g(e,t)}function h(e,t,n=`lg`){return`<span class="acc-avatar ${n}" style="background:${e}">${u(t)}</span>`}async function g(e,n){let r=await o();e.innerHTML=`
  ...
  `,t.querySelectorAll(`.swatch`).forEach(e=>e.addEventListener(`click`,()=>{l=e.dataset.color,t.querySelectorAll(`.swatch`).forEach(t=>t.classList.toggle(`on`,t===e))}));let h=t.querySelector(`#acc-pin`),g=t.querySelector(`#acc-pin2`),v=t.querySelector(`#pin-hint`);function y(){return u=h.value.trim(),m=g.value.trim(),!u&&!m?(v.textContent=`Leave both empty to skip the PIN`,!1):/^\d{4}$/.test(u)?u===m?(v.textContent=`PIN ready — you will enter it when opening this profile`,!0):(v.textContent=`PINs do not match`,!1):(v.textContent=`PIN must be exactly 4 digits`,!1)}h.addEventListener(`input`,y),g.addEventListener(`input`,y),t.querySelector(`#acc-back-btn`).addEventListener(`click`,()=>{s.state.accountFlow?.cameFromPicker?(s.state.accountFlow={mode:`picker`},s.go(`accounts`)):(s.state.accountFlow=null,s.go(`onboarding`))}),t.querySelector(`#acc-create-btn`).addEventListener(`click`,async()=>{let a=t.querySelector(`#acc-name`).value.trim();if(!a){s.toast(`Enter a profile name`,!0);return}let p=y(),h=u||m;if(h&&!p){s.toast(v.textContent,!0);return}let g=h?await i(u):null;if(!_)return;let b=await n({name:a,color:l,pinHash:g});if(!_)return;let x=(await o()).filter(e=>e.id!==b.id);for(let t of x)await e(t.id)||await r(t.id);if(!_)return;f(b.id),s.state.account=b;let S={...d().tutorialDoneAccounts||{}};S[b.id]=!1,c({tutorialAccountId:b.id,tutorialDone:!1,tutorialDoneAccounts:S,tourSeen:[]}),s.toast(`Welcome, ${b.name}!`),s.go(`tutorial`)})}var b=!1;async function x(e,n){b=!0;let{accountId:r}=n.state.accountFlow,i=await t(r);if(!i){n.state.accountFlow={mode:`picker`},n.go(`accounts`);return}let o=``;e.innerHTML=`
    <div class="acc-screen">
      <div class="acc-brand"><span class="mark">${a(`logo`)}</span>Quizard</div>
  ...
  `;let c=[...e.querySelectorAll(`.pin-dot`)];function d(){c.forEach((e,t)=>e.classList.toggle(`filled`,t<o.length))}async function p(){let t=await s(o,i.pinHash);if(b){if(t.ok)t.upgrade&&l(i.id,{pinHash:t.upgrade}).catch(()=>{}),f(i.id),n.state.account=i,n.state.accountFlow=null,n.toast(`Welcome back, ${i.name}`),n.go(`library`);else{let t=e.querySelector(`.pin-pad`);t.classList.add(`shake`),setTimeout(()=>t.classList.remove(`shake`),400),o=``,d(),n.toast(`Wrong PIN`,!0)}}}e.querySelectorAll(`.pin-key[data-k]`).forEach(e=>e.addEventListener(`click`,()=>{o.length>=4||(o+=e.dataset.k,d(),o.length===4&&setTimeout(p,120))})),e.querySelector(`#pin-del`).addEventListener(`click`,()=>{o=o.slice(0,-1),d()}),e.querySelector(`#pin-cancel`).addEventListener(`click`,()=>{n.state.accountFlow={mode:`picker`},n.go(`accounts`)})}export{m as render,v as unmount};